...
func TestGoChannelEventBus_Publish(t *testing.T) {
	...
	received := make(chan member_domain_events.IndividualMemberRegisteredEvent)
	...
	go func() {
		...
		assert.Equal(t, member_domain_events.IndividualMemberRegisteredEventName, domain_event_name)

		imrEvent, err := member_domain_events.NewIndividualMemberRegisteredEventFromJsonBytes(msg.Payload)
		assert.NoError(t, err)

		received <- imrEvent
	}()

	bus.Publish(member_domain_events.NewIndividualMemberRegisteredEvent(member_domain.MemberId(1)))

	select {
	case event := <-received:
		assert.Equal(t, member_domain_events.IndividualMemberRegisteredEventName, event.GetName())
